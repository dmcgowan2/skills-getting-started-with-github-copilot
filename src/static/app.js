    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0 
          ? details.participants.map(p => `<li><span class="participant-name">${p}</span><button class="delete-participant" data-activity="${name}" data-participant="${p}" aria-label="Remove ${p}">×</button></li>`).join("")
        ...
        deleteButtons.forEach(button => {
          button.addEventListener("click", async (event) => {
            ...
            try {
              ...
              if (response.ok) {
                messageDiv.textContent = result.message || `${participant} has been unregistered.`;
